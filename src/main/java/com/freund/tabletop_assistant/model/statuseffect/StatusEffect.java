...
public enum StatusEffect {
    ...
    PHYSICAL_IMMUNITY("Immune to Bludgeoning, Piercing and Slashing damage.", List.of(StatusEffect.BLUDGEONING_IMMUNITY, StatusEffect.PIERCING_IMMUNITY, StatusEffect.SLASHING_IMMUNITY)),
    PHYSICAL_RESISTANCE("Resistant to Bludgeoning, Piercing and Slashing damage.", List.of(StatusEffect.BLUDGEONING_RESISTANCE, StatusEffect.PIERCING_RESISTANCE, StatusEffect.SLASHING_RESISTANCE)),
    PHYSICAL_VULNERABILITY("Vulnerable to Bludgeoning, Piercing and Slashing damage.", List.of(StatusEffect.BLUDGEONING_VULNERABILITY, StatusEffect.PIERCING_VULNERABILITY, StatusEffect.SLASHING_VULNERABILITY)),
    ...
    CHARM_STATUS("Charmed.", List.of()),
    // FRIGHTENED_STATUS
    DULL("Can't take Reactions.", List.of()),
    PRONE("Disadvantage on Attack Rolls. Melee attacks against the creature have Advantage. Ranged attacks against the creature have Disadvantage. Crawling costs double movement. Can spend half its maximum Movement Speed to stand up.", List.of(StatusEffect.UNFOCUSED)),
    PARALYZED("Incapacitated and can't move or speak. Automatically fails Strength and Dexterity Saving Throws. Attack Rolls against the affected entity have Advantage. Melee attacks against the creature are always Critical Hits", List.of(StatusEffect.INCAPACITATED, StatusEffect.IMMOBILE, StatusEffect.VULNERABLE)), // TODO silenced?
    RAGE("Deals additional damage with melee and improvised weapons. Damage amount depends on level. Resistant to physical damage, Advantage on Strength Checks and Saving Throws. Can't cast or concentrate on spells. Rage ends early if the barbarian hasn't attacked an enemy or taken damage since their last turn.", List.of(StatusEffect.PHYSICAL_RESISTANCE)),
    BLINDED("Disadvantage on Attack Rolls. Ranged attacks and spells have a range of 10 ft. Unaffected by Illusion spells. Attack Rolls against the affected entity have Advantage.", List.of(StatusEffect.UNFOCUSED, StatusEffect.VULNERABLE)),
    UNCONSCIOUS("Paralyzed and Prone. Unaware of surroundings. Drops whatever it's holding.", List.of(StatusEffect.PARALYZED, StatusEffect.PRONE)),
    ...
    ARMS_OF_HADAR("Can't take Reactions", List.of(StatusEffect.DULL)),
    ...
    AURA_OF_COURAGE("The paladin and any nearby allies in a 10 ft radius can't be Frightened.", List.of()),
    AURA_OF_DEVOTION("The paladin and any nearby allies in a 10 ft radius can't be Charmed.", List.of()),
    ...
    BANISHED("Banished from this plane of existance. Incapacitated, can't move or be targeted", List.of(StatusEffect.INCAPACITATED, StatusEffect.IMMOBILE)),
    ...
    BLACK_TENTACLES("Restrained by dark tendrils. Disadvantage on Attack Rolls. Affected entity can't move and takes 3d6 Bludgeoning damage per turn. Attack Rolls against the affected entity have Advantage. Strength Saving Throw to end the condition.", List.of(StatusEffect.IMMOBILE)),
    BLADE_WARD("Resistant to Bludgeoning, Piercing, and Slashing damage.", List.of(StatusEffect.PHYSICAL_RESISTANCE)),
    ...
    BLINKED_TO_THE_ETHEREAL_PLANE("Affected entity is in the Ethereal Plane, where it can't be harmed, seen, or affected. Can't move or interact with anything in the world, but can choose to teleport up to 20ft.", List.of(StatusEffect.IMMOBILE)),
    BLUDGEON_THE_WEAK("Affected entity is Vulnerable to Bludgeoning damage for 3 turns, or until it takes damage.", List.of()),
    BLURRED("Attack Rolls against the affected entity have Disadvantage, unless they do not rely on sight or can see through illusions.", List.of()),
    ...
    BURNING("Takes 1d4 Fire damage per turn. Removed when Helped. Immune from if Wet.", List.of()),
    CALL_LIGHTNING("Can cast Call Lightning without expending a spell slot.", List.of()),
    CALMED("Can't be Charmed or Frightened, and can't Rage", List.of()),
    CATS_GRACE("Advantage on Dexterity Checks. Takes half falling damage.", List.of()),
    CHANNEL_DIVINITY_CLOAK_OF_SHADOWS("Advantage on Attack Rolls. Attack Rolls against the affected entity have Disadvantage. Condition ends if affected entity attacks, casts a spell, takes an Action, or takes damage.", List.of()),
    CHARMED("Can't attack spellcaster. Spellcaster has Advantage on Charisma Checks in dialogue with the affected entity.", List.of(StatusEffect.CHARM_STATUS)),
    ...
    CLOAK_OF_SHADOWS("Advantage on Attack Rolls. Attack Rolls against the affected entity have Disadvantage. Attacking, casting spells, or entering a brightly lit area ends this condition.", List.of()),
    COCOONED("Trapped in a spider's web. Incapacitated and can't move. Condition ends upon taking damage or making a successful Strength Saving Throw.", List.of(StatusEffect.INCAPACITATED, StatusEffect.IMMOBILE)),
    ...
    COMMAND_APPROACH("Incapacitated. Must move toward the spellcaster by the shortest route possible.", List.of(StatusEffect.INCAPACITATED)),
    COMMAND_DROP("Incapacitated. Must drop whatever it is holding.", List.of(StatusEffect.INCAPACITATED)),
    COMMAND_FLEE("Incapacitated. Must move away from the spellcaster by the fastest means possible.", List.of(StatusEffect.INCAPACITATED)),
    COMMAND_GROVEL("Incapacitated and can't move. Must drop Prone.", List.of(StatusEffect.INCAPACITATED, StatusEffect.IMMOBILE, StatusEffect.PRONE)),
    COMMAND_HALT("Incapacitated and can't move.", List.of(StatusEffect.INCAPACITATED, StatusEffect.IMMOBILE)),
    ...
    CONTAGION_BLINDING_SICKNESS("Blinded. Disadvantage on Wisdom Checks and Wisdom Saving Throws.", List.of(StatusEffect.BLINDED)),
    CONTAGION_FILTH_FEVER("Disadvantage on Strength Checks, Strength Saving Throws and Strength Attack Rolls.", List.of()),
    CONTAGION_FLESH_ROT("Vulnerable to all damage. Disadvantage on Charisma Checks.", List.of()),
    ...
    CURSE_OF_THE_DIRE_RAVEN("Marked by the feather of a dire raven. Attack Rolls against the affected entity have Advantage.", List.of()),
    ...
    DAZED("Disadvantage on Wisdom Saving Throws, can't take Reactions, and loses the Dexterity bonus to their Armour Class. Removed when Helped.", List.of(StatusEffect.DULL)),
    DEAD("This creature is dead.", List.of()),
    DEAFENED("Affected entity can't hear. Attack Rolls against the affected entity have Advantage. Unaffected by Bardic Inspiration.", List.of()),
    ...
    DOMINATED("Affected entity is under the control of the caster. Each time it takes damage, it can break the spell by making a successful Wisdom Saving Throw. ", List.of(StatusEffect.CHARM_STATUS)),
    DOWNED("Unconscious. Death Saving Throws must be rolled each turn. Any damage received counts as one failure. Can be removed by healing or when Helped.", List.of(StatusEffect.UNCONSCIOUS)),
    ...
    ELEMENTAL_AFFINITY_ACID_RESISTANCE("Resistant to Acid damage.", List.of()),
    ELEMENTAL_AFFINITY_COLD_RESISTANCE("Resistant to Cold damage.", List.of()),
    ELEMENTAL_AFFINITY_FIRE_RESISTANCE("Resistant to Fire damage.", List.of()),
    ELEMENTAL_AFFINITY_LIGHTNING_RESISTANCE("Resistant to Lightning damage.", List.of()),
    ELEMENTAL_AFFINITY_POISON_RESISTANCE("Resistant to Poison damage.", List.of()),
    ...
    EXHAUSTED_2("Movement Speed halved.", List.of(StatusEffect.EXHAUSTED_1)),
    EXHAUSTED_3("Disadvantage on Attack Rolls and Saving Throws.", List.of(StatusEffect.EXHAUSTED_2)),
    EXHAUSTED_4("HP maximum halved.", List.of(StatusEffect.EXHAUSTED_3)),
    EXHAUSTED_5("Can't move.", List.of(StatusEffect.EXHAUSTED_4, StatusEffect.IMMOBILE)),
    EXPEDITIOUS_RETREAT("Dash available as a Bonus Action.", List.of()),
    EYEBITE_SICKENED("Disadvantage on Attack Rolls and Ability Checks.", List.of()),
    ...
    FEIGNING_DEATH("Sunk into a deep magical coma. Incapacitated and can't move. Resistant to all damage except Psychic damage.", List.of(StatusEffect.INCAPACITATED, StatusEffect.IMMOBILE)), // TODO resistances
    ...
    FLESH_TO_STONE("Affected entity can't move as its body begins to petrify over the course of 3 turns. Attack Rolls against it have Advantage, while the entity's Attack Rolls and Dexterity Saving Throws have Disadvantage.", List.of(StatusEffect.IMMOBILE)),
    ...
    FRENZIED("In Rage. Can use Frenzied Strike and Enraged Throw. When Frenzy ends, suffers one level of exhaustion.", List.of(StatusEffect.RAGE)),
    ...
    FRIENDS("Advantage on Charisma Checks directed at the affected entity.", List.of(StatusEffect.CHARM_STATUS)),
    FRIGHTENED("Can't move. Disadvantage on Ability Checks and Attack Rolls.", List.of(StatusEffect.IMMOBILE)),
    FROZEN("Incapacitated. Vulnerable to Bludgeoning, Thunder, and Force damage. Resistant to Fire damage. Immune to Burning. If it is dealt Bludgeoning, Thunder, or Force damage, the ice shatters, ending the condition.", List.of()),
    GAPING_WOUNDS("Attacks against this creature deal an additional 2 Piercing damage. Removed by healing.", List.of()),
    GASEOUS_FORM("Transformed into a cloud of mist. Resistant to all damage. Advantage on Constitution, Dexterity, and Strength Saving Throws. Becomes Tiny in size. Can't attack, cast spells, or speak. The affected entity's Strength and Dexterity scores are set to 10 while in this form.", List.of()),
    GOADED("Must attack the goading creature, if possible. Disadvantage against targets other than the spellcaster.", List.of()),
    GRAPPLED("Can't move. Condition ends if the grappler is Incapacitated or an effect removes the grappled creature from the reach of the grappler, such as when a creature is hurled away by the thunderwave spell.", List.of(StatusEffect.IMMOBILE)),
    ...
    GREATER_INVISIBLITY("Advantage on Attack Rolls. Attack Rolls against the affected entity have Disadvantage.", List.of()),
    ...
    HEROES_FEAST("HP maximum increased by 12 points. Gains 12 HP. Immune to poison. Can't be Frightened. Advantage on Wisdom Saving Throws.", List.of()),
    HEROISM("Can't be Frightened. Receive 5 temporary HP at the start of each turn.", List.of()),
    ...
    HIDEOUS_LAUGHTER("Incapacitated and can't move. Prone. Disadvantage on Strength and Dexterity Saving Throws. At the end of its turn or when it takes damage, affected entity makes a Wisdom Saving Throw to end the condition.", List.of(StatusEffect.INCAPACITATED, StatusEffect.IMMOBILE, StatusEffect.PRONE)),
    HOLD_MONSTER("Incapacitated and can't move. Automatically fails all Strength and Dexterity Saving Throws. Attack Rolls against the entity have Advantage. Melee attacks against the entity are always Critical Hits.", List.of(StatusEffect.INCAPACITATED, StatusEffect.IMMOBILE)),
    HOLD_PERSON("Incapacitated and can't move. Automatically fails all Strength and Dexterity Saving Throws. Attack Rolls against the entity have Advantage. Melee attacks against the entity are always Critical Hits.", List.of(StatusEffect.INCAPACITATED, StatusEffect.IMMOBILE)),
    ...
    HYPNOTIC_GAZE("Incapacitated and can't move. Spellcaster has Advantage on Ability Checks in dialogue. Removed upon taking damage. Duration can be extended with Maintain Hypnotic Gaze", List.of(StatusEffect.INCAPACITATED, StatusEffect.IMMOBILE, StatusEffect.CHARM_STATUS)),
    HYPNOTIZED("Incapacitated and can't move. Removed by taking damage, being Shoved, or Helped", List.of(StatusEffect.INCAPACITATED, StatusEffect.IMMOBILE, StatusEffect.CHARM_STATUS)),
    ...
    INVISIBLE("Advantage on Attack Rolls. Attack Rolls against affected entity have Disadvantage. Invisibility ends early if the invisible entity attacks, casts a spell, interacts with an object, takes an Action or Bonus Action, or is damaged.", List.of()),
    INVULNERABLE("Immune to all damage. Can still be afflicted with status effects.", List.of()),
    IRRESISTIBLE_DANCE("Can't move or take Actions. Disadvantage on Attack Rolls and Dexterity Saving Throws. Attack Rolls against the affected entity have Advantage.", List.of(StatusEffect.IMMOBILE, StatusEffect.CHARM_STATUS)),
    ...
    LETHARGIC("Consumed by exhaustion in the aftermath of Haste. Incapacitated and can't move.", List.of(StatusEffect.INCAPACITATED, StatusEffect.IMMOBILE)),
    LONGSTRIDER("Movement Speed increased by 10 ft.", List.of()),
    LURED("Attracted to a harpy's luring melody. Must move toward the harpy by the shortest route possible. Makes a 13 DC Wisdom Saving Throw each turn. Being attacked or Shoved will remove the condition.", List.of(StatusEffect.CHARM_STATUS)),
    ...
    MAIMED("Can't move. Disadvantage on Dexterity Saving Throws. Removed by healing.", List.of(StatusEffect.IMMOBILE)),
    ...
    NATURES_WRATH("Can't move. Disadvantage on Attack Rolls and Dexterity Saving Throws. Attack Rolls against the affected entity have Advantage.", List.of(StatusEffect.IMMOBILE)), // TODO remove and change spell description to "Restrain"
    NAUSEOUS("Can't take Actions.", List.of()),
    NEWBORN_ZOMBIE("Takes 1 Necrotic damage per turn.", List.of()),
    OFF_BALANCE("Disadvantage on Strength and Dexterity Ability Checks. Attack Rolls against the affected entity have Advantage. Removed by taking damage or when Helped.", List.of()),
    ...
    PATIENT_DEFENCE(" Advantage on Dexterity Saving Throws. Attack Rolls against the affected entity have Disadvantage.", List.of()),
    PETRIFIED("Incapacitated, can't move or speak, and is unaware of its surroundings. Automatically fails Strength and Dexterity Saving Throws. Resistance to all damage, immunity to poison and disease.", List.of(StatusEffect.INCAPACITATED, StatusEffect.IMMOBILE)), //TODO silenced?
    ...
    PHANTASMAL_KILLER("Can't move and takes 4d10 + 1d10 per upcast level Psychic damage per turn. Disadvantage on Ability Checks and Attack Rolls.", List.of(StatusEffect.IMMOBILE)),
    PLANAR_BINDING("Affected entity follows and fights for the one who has bound it. Every time it takes damage, it may break the caster's control over it with a successful Wisdom Saving Throw.", List.of(StatusEffect.CHARM_STATUS)),
    POISONED("Disadvantage on Attack Rolls and Ability Checks. Takes 1d4 Poison damage per turn.", List.of()),
    ...
    PROTECTION_FROM_EVIL_AND_GOOD("Can't be Charmed or Frightened. Aberrations, Celestials, Elementals, Fey, Fiends, and Undead have Disadvantage against the affected entity.", List.of()),
    ...
    RAGE_BOAR("In Rage. Becomes Enlarged. Deals an additional 2 Slashing damage. Resistant to physical damage, Advantage on Strength Checks and Saving Throws. Can't cast or concentrate on spells. Rage ends early if the boar hasn't attacked an enemy or taken damage since their last turn.", List.of(StatusEffect.RAGE)),
    RAGE_BEAR_HEART("In Rage. Can use Unrelenting Ferocity. Resistant to all damage types except Psychic.", List.of(StatusEffect.RAGE)),
    RAGE_EAGLE_HEART("In Rage. Can use Diving Strike. Can use Dash as a Bonus Action. Opportunity Attacks against the affected entity have Disadvantage.", List.of(StatusEffect.RAGE)),
    RAGE_ELK_HEART("In Rage. Can use Primal Stampede. Movement Speed increased by 15 ft.", List.of(StatusEffect.RAGE)),
    RAGE_TIGER_HEART("In Rage. Can use Tiger's Bloodlust. Jump distance increased by 15 ft.", List.of(StatusEffect.RAGE)),
    RAGE_WOLF_HEART("In Rage. Can use Inciting Howl. Allies have melee Advantage against enemies within 7ft of affected entity.", List.of(StatusEffect.RAGE)),
    ...
    RECKLESS_ATTACK("Advantage on Attack Rolls. Attack Rolls against the affected entity also have Advantage.", List.of()),
    ...
    RESTRAINED("Can't move. Disadvantage on Attack Rolls and Dexterity Saving Throws. Attack Rolls against the affected entity have Advantage.", List.of(StatusEffect.IMMOBILE)),
    ...
    SHOCKED("Can't take Reactions. Disadvantage on Ability Checks and Saving Throws using Dexterity.", List.of(StatusEffect.DULL)),
    SHOCKING_GRASP("Can't take Reactions.", List.of(StatusEffect.DULL)),
    ...
    STAGGERED("Can't take Reactions.", List.of(StatusEffect.DULL)),
    // STEP_OF_THE_WIND ??
    STONESKIN("Resistant to Bludgeoning, Piercing, and Slashing damage.", List.of(StatusEffect.PHYSICAL_RESISTANCE)),
    STUNNED("Incapacitated and can't move. Automatically fails Strength and Dexterity Saving Throws.", List.of(StatusEffect.INCAPACITATED, StatusEffect.IMMOBILE)), // Attack Rolls against affected entity have Advantage ??
    ...
    SURPRISED("Can't move, take Actions or Reactions.", List.of(StatusEffect.IMMOBILE)),
    ...
    TURNED("Incapacitated. Must run as far away from caster as it can. Condition ends upon taking damage.", List.of(StatusEffect.INCAPACITATED)),
    ...
    VISCIOUS_MOCKERY("Disadvantage on next Attack Roll.", List.of()),
    ...
    WARDING_BOND_ALLY("Gains a +1 bonus to Armour Class and Saving throws. Resistant to all damage. Each time the affected entity takes damage, the caster takes the same amount of damage.", List.of()),
    ...
    WEAK_GRIP("Disadvantage on Attack rolls and Strength Saving throws. Removed when Helped.", List.of()),
    ...
    WET("Immune to Burning. Resistant to Fire damage. Vulnerable to Lightning and Cold damage.", List.of()),
